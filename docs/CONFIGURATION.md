...
| `LNURL_MODE_REQUESTS_PER_IP_PER_MINUTE` | Optional | `10` | Per-client-IP budget shared by `POST /lnurlpay/{pubkey}/mode` and the country lookups the register/recover routes make for Enhanced accounts. IPv6 clients are keyed by /64 prefix. The counter is in-process, so the effective budget is per replica. Outside `DEPLOYMENT_ENV=local` a request with no trusted `x-real-ip` gets no budget. |
| `LNURL_COUNTRY_LOOKUPS_PER_DAY` | Optional | `5000` | Daily cap on country lookups across all client IPs (per replica), protecting the shared proxycheck quota from traffic that stays under every per-IP budget. Over budget, mode writes still succeed and stored evidence stops refreshing until the window resets. |
