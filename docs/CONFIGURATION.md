...
| `LNURL_NETWORK` | Optional | `mainnet` | Spark network passed to the Spark client, such as `mainnet`, `testnet`, or `regtest`. |
...
| `LNURL_BLINK_GRAPHQL_ENDPOINT` | Optional | `https://api.blink.sv/graphql` | Blink public GraphQL endpoint used for Blink provider invoice and payment-status calls. |
