...
4. Start the local PostgreSQL-backed server stack:
   ...
   `scripts/start-local-stack.sh` starts PostgreSQL with Docker Compose, builds `target/debug/lnurl-server` if needed, and runs the server on `127.0.0.1:8080` with regtest settings. Set `LNURL_BLINK_GRAPHQL_ENDPOINT` first when using `DEPLOYMENT_ENV=local`; otherwise the runtime defaults to `production`.
