...
ssh ${ADDITIONAL_SSH_OPTS} ${DOCKER_HOST_USER}@${DOCKER_HOST_IP} \
  "cd ${REPO_PATH}; COMPOSE_PROJECT_NAME=${REPO_PATH} docker compose -f docker-compose.yml -f ci/docker-compose.ci.yml up --build --abort-on-container-exit --exit-code-from e2e-tests e2e-tests"
...
docker compose -f docker-compose.yml -f ci/docker-compose.ci.yml down --remove-orphans --timeout 1
