...
gcloud_ssh "cd ${REPO_PATH}; export TMP_ENV_CI=tmp.env.ci; export COMPOSE_PROJECT_NAME=$(basename ${REPO_PATH}); source .envrc || true; docker compose -f docker-compose.yml -f ci/docker-compose.ci.yml up --build --abort-on-container-exit --exit-code-from ${TEST_CONTAINER} ${TEST_CONTAINER} 2>&1"
...
gcloud_ssh "cd ${REPO_PATH}; export COMPOSE_PROJECT_NAME=$(basename ${REPO_PATH}); docker compose -f docker-compose.yml -f ci/docker-compose.ci.yml down --remove-orphans --timeout 1"
